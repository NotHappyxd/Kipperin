...
import me.happy.hcf.faction.event.FactionRelationCreateEvent;
...
public class FactionAllyArgument extends CommandArgument {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (ConfigurationService.MAX_ALLIES <= 0) {
        ...
        if (allied.size() >= ConfigurationService.MAX_ALLIES) {
            sender.sendMessage(ChatColor.RED + "Your faction already has reached the alliance limit, which is " + ConfigurationService.MAX_ALLIES + '.');
            return true;
        }

        if (targetFaction.getAllied().size() >= ConfigurationService.MAX_ALLIES) {
            sender.sendMessage(targetFaction.getDisplayName(sender) + ChatColor.RED + " has reached their maximum alliance limit, which is " +
                    ConfigurationService.MAX_ALLIES + '.');
